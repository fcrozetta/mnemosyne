...
def test_feature_flags_default_to_off(monkeypatch) -> None:
    ...
    assert settings.access_policy_enabled is False
...
def test_feature_flags_parse_truthy_values(monkeypatch) -> None:
    ...
    assert settings.access_policy_enabled is True
...
def test_parse_bool_rejects_unknown_values() -> None:
    try:
        parse_bool_env("maybe", name="MNEMOSYNE_ACCESS_POLICY_ENABLED")
    except ValueError as exc:
        assert "MNEMOSYNE_ACCESS_POLICY_ENABLED" in str(exc)
    else:
